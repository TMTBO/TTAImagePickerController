...
protocol TTAAssetPickerViewControllerDelegate: class {
	func assetPickerController(_ picker: TTAAssetPickerViewController,
							   didFinishPicking assets: [PHAsset])
}
...
class TTAAssetPickerViewController: UIViewController {
	...
	fileprivate let collectionView = UICollectionView(frame: .zero,
													  collectionViewLayout: TTAAssetCollectionViewLayout())
...
fileprivate extension TTAAssetPickerViewController {
	...
	func prepareCollectionView() {
		...
		collectionView.register(TTAAssetCollectionViewCell.self,
								forCellWithReuseIdentifier: "\(TTAAssetCollectionViewCell.self)")
		collectionView.register(TTACameraCell.self,
								forCellWithReuseIdentifier: "\(TTACameraCell.self)")
	}
	...
	func prepareCancelItem() {
		...
		let cancelItem = UIBarButtonItem(barButtonSystemItem: .cancel,
										 target: self,
										 action: #selector(didClickCancelItem))
		self.navigationItem.rightBarButtonItem = cancelItem
	}

	func prepareToolBar() {
		previewItem = UIBarButtonItem(title: Bundle.localizedString(for: "Preview"),
									  style: .plain,
									  target: self,
									  action: #selector(didClickPreviewItem))
		previewItem.isEnabled = false
		deleteItem = UIBarButtonItem(barButtonSystemItem: .trash,
									 target: self,
									 action: #selector(didClickDeleteItem))
		deleteItem.isEnabled = false
		doneItem = UIBarButtonItem(barButtonSystemItem: .done,
								   target: self,
								   action: #selector(didClickDoneItem))
		...
		countLabel = TTASelectCountLabel(frame: CGRect(x: 0,
													   y: 0,
													   width: countLabelWH,
													   height: countLabelWH))
		...
		if allowDeleteImage {
			toolBarItems.insert(deleteItem,
								at: 0)
		}
	...
	func scrollTo(_ index: Int) {
		DispatchQueue.main.async { [weak self] in
			...
			if self.assetCount() <= Int(
					TTAAssetCollectionViewLayout.TTAAssetCollectionViewLayoutConst.correctColumNum) {
				return
			}
			//            self.collectionView.scrollToItem(at: IndexPath(item: index, section: 0), at: .centeredVertically, animated: false)

			guard let attribute = self.collectionView.collectionViewLayout.layoutAttributesForItem(
					at: IndexPath(item: index,
								  section: 0)) else {
				return
			}
	...
	func setup(assetCell cell: TTAAssetCollectionViewCell,
			   indexPath: IndexPath) {
		guard let currentAsset = asset(at: indexPath) else {
			return
		}
		...
		album.requestThumbnail(with: indexPath.item,
							   size: cell.bounds.size.toPixel()) { (image) in
			guard let image = image, cell.tag == tag else {
				return
			}
	...
	func lightupCell(with index: Int,
					 isPreview: Bool) {
		let correctIndex = isPreview ? album.index(for: selected[
				index
				]) : index
		let indexPath = IndexPath(item: correctIndex,
								  section: 0)
		scrollTo(correctIndex)
		guard let cell = collectionView.cellForItem(at: indexPath) as? TTAAssetCollectionViewCell else {
			return
		}
		cell.lightUp()
	}

	func showPreviewViewController(from index: Int,
								   isPreview: Bool) {
		guard let currentAsset = asset(at: IndexPath(item: index,
													 section: 0)) else {
			return
		}
		...
		navigationController?.pushViewController(previewVc,
												 animated: true)
	}
...
extension TTAAssetPickerViewController {
	...
	func asset(for cell: UICollectionViewCell) -> PHAsset? {
		guard let indexPath = collectionView.indexPath(for: cell),
			  let operateAsset = asset(at: indexPath) else {
			return nil
		}
		return operateAsset
	}
...
extension TTAAssetPickerViewController {
	...
	func didClickPreviewItem() {
		showPreviewViewController(from: 0,
								  isPreview: true)
	}
...
extension TTAAssetPickerViewController: UICollectionViewDataSource {
	...
	func collectionView(_ collectionView: UICollectionView,
		...
		guard allowTakePicture && album.albumInfo.canShowCamera && indexPath.item == assetCount() else {
			let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "\(TTAAssetCollectionViewCell.self)",
														  for: indexPath)
			return cell
		}
		let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "\(TTACameraCell.self)",
													  for: indexPath)
		return cell
	}
}
...
extension TTAAssetPickerViewController: UICollectionViewDelegate {
	...
	func collectionView(_ collectionView: UICollectionView,
		...
		showPreviewViewController(from: indexPath.item,
								  isPreview: false)
	}
	...
	func collectionView(_ collectionView: UICollectionView,
		...
		guard let cell = cell as? TTAAssetCollectionViewCell else {
			return
		}
		setup(assetCell: cell,
			  indexPath: indexPath)
	}
}
...
extension TTAAssetPickerViewController: TTAAssetCollectionViewCellDelegate {
	func canOperateCell(cell: TTAAssetCollectionViewCell) -> (canOperate: Bool, asset: PHAsset?) {
		guard let operateAsset = asset(for: cell) else {
			return (false,
					nil)
		}
	...
	func assetCell(_ cell: TTAAssetCollectionViewCell,
				   asset: PHAsset,
				   isSelected: Bool) {
		operateAsset(asset,
					 isSelected: isSelected)
...
extension TTAAssetPickerViewController: TTAPreviewViewControllerDelegate {
	...
	func previewViewController(_ previewVc: TTAPreviewViewController,
							   didFinishPicking assets: [PHAsset]) {
		delegate?.assetPickerController(self,
										didFinishPicking: assets)
	}

	func previewViewController(_ previewVc: TTAPreviewViewController,
							   backToAssetPickerControllerWith currentIndex: Int,
							   selectedAsset: [PHAsset]) {
		lightupCell(with: currentIndex,
					isPreview: previewVc.album == nil)
		guard self.selected != selectedAsset else {
			return
		}
		DispatchQueue.global().async { [weak self] in
			...
			let indexPaths = assets.map {
				IndexPath(item: self.album.index(for: $0),
						  section: 0)
			}
			self.selected = selectedAsset
